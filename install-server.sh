...
get_download_urls() {
    ...
    # Prefer GitHub Releases (does not rely on LFS raw endpoints)
    ...
    # Fallback to raw (may serve LFS pointer if quota exceeded)
    echo "https://raw.githubusercontent.com/scantrancher/terminal-anywhere/main/latest/${binary_file}"
}

# Quick check to ensure downloaded file is a real binary, not an LFS pointer or HTML
is_valid_binary() {
    local path="$1"
    # Empty or missing file
    [ -f "$path" ] && [ -s "$path" ] || return 1
    # Detect LFS pointer text files
    if head -n 1 "$path" | grep -qi "git-lfs.github.com/spec/v1"; then
        return 1
    fi
    # Check magic bytes for ELF or Mach-O 64-bit
    if command -v xxd >/dev/null 2>&1; then
        local magic
        magic=$(xxd -p -l 4 "$path" 2>/dev/null | tr '[:lower:]' '[:upper:]')
        case "$magic" in
            7F454C46) return 0 ;;      # ELF
            CFFAEDFE) return 0 ;;      # Mach-O 64 (LE)
            CEFAEDFE) return 0 ;;      # Mach-O 32 (LE)
            FEEDFACF) return 0 ;;      # Mach-O 64 (BE)
            FEEDFACE) return 0 ;;      # Mach-O 32 (BE)
        esac
        # If magic didn't match, still allow if file has exec bit and is not text/html
    fi
    # Heuristic: reject small text files
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo "    TA_RELEASE_TAG=v1.24.4 curl -L https://raw.githubusercontent.com/scantrancher/terminal-anywhere/main/install-server.sh | bash"
    echo "    curl -L https://raw.githubusercontent.com/scantrancher/terminal-anywhere/main/install-server.sh | bash -s -- --tag v1.24.4"
    exit 0
fi
